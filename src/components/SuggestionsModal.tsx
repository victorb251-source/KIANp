import React from 'react';
import { XMarkIcon, LightBulbIcon } from './Icon';
import Loader from './Loader';

interface SuggestionsModalProps {
  isOpen: boolean;
  onClose: () => void;
  suggestions: string | null;
  isLoading: boolean;
  error: string | null;
}

const SuggestionsModal: React.FC<SuggestionsModalProps> = ({ isOpen, onClose, suggestions, isLoading, error }) => {
  if (!isOpen) return null;

  const renderContent = () => {
    if (isLoading) {
      return (
        <div className="flex flex-col items-center justify-center h-full">
          <Loader message="Analisando seus erros..." />
        </div>
      );
    }
    if (error) {
      return (
        <div className="bg-accent-error/20 border border-accent-error text-accent-error p-3 rounded-md text-left text-sm" role="alert">
          <p className="font-bold">Erro ao gerar sugestões</p>
          <p>{error}</p>
        </div>
      );
    }
    if (suggestions) {
      return (
        <div className="text-text-secondary leading-relaxed space-y-4 whitespace-pre-wrap">
          {suggestions}
        </div>
      );
    }
    return null;
  };

  return (
    <div className="fixed inset-0 bg-black/60 z-40 flex items-center justify-center p-4" onClick={onClose}>
      <div
        className="bg-bg-primary border border-border-color rounded-xl shadow-2xl max-w-2xl w-full max-h-[80vh] flex flex-col animate-fade-in"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="p-6 relative flex-shrink-0 border-b border-border-color">
          <h2 className="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-orange-500 flex items-center gap-3">
            <LightBulbIcon className="w-8 h-8"/>
            Sugestões de Estudo
          </h2>
          <p className="text-sm text-text-tertiary mt-1">A IA analisou seus erros para criar um plano de melhoria personalizado.</p>
          <button onClick={onClose} className="absolute top-4 right-4 text-text-tertiary hover:text-text-primary transition">
            <XMarkIcon className="w-7 h-7" />
          </button>
        </div>
        
        <div className="px-6 pb-6 pt-4 overflow-y-auto flex-grow">
          {renderContent()}
        </div>
      </div>
    </div>
  );
};

export default SuggestionsModal;